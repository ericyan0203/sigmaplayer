...
/*__TMOD*/		
		...
		st->codec->chunk_size  = v ;

		temp = avio_rl32(pb);
        if (temp != MKTAG('V', 'I', 'D', 'O')) {
        fail1:
            av_log(s, AV_LOG_WARNING, "Unsupported stream type %08x\n", v);
            goto skip;
        }

		st->codec->media_object_format = temp;
        st->codec->codec_tag = avio_rl32(pb); //fource 
        st->codec->codec_id  = ff_codec_get_id(ff_rm_codec_tags,
                                               st->codec->codec_tag);
        ...
        st->codec->bpp = avio_rb16(pb);
        //avio_skip(pb, 4); // always zero?
        st->codec->pad_width =  avio_rb16(pb);
		st->codec->pad_height = avio_rb16(pb);
		...
		st->codec->fps = fps;
		 ...
		 av_log(s, AV_LOG_WARNING,
                   "Eric FFMPEG pad %xX%x bpp %x fps %x\n",
               st->codec->pad_width,st->codec->pad_height, st->codec->bpp,st->codec->fps );
