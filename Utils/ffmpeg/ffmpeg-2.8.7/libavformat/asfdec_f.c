...
static int asf_read_stream_properties(AVFormatContext *s, int64_t size)
{
	...
	VC1Frame  *pvc1Frm =  &(s->VC1Data); //__TMOD
    ...
    if (type == AVMEDIA_TYPE_AUDIO) {
        ...
        if (sizeX > 40) {
			...
			if ((st->codec->codec_id  == AV_CODEC_ID_VC1) || (st->codec->codec_id  == AV_CODEC_ID_WMV3))
			{   
                  pvc1Frm->firstFrame = 1;
            } 	
        }
...
static int asf_parse_packet(AVFormatContext *s, AVIOContext *pb, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        if (asf_st->pkt.size != asf_st->packet_obj_size ||
			...
			//__TMOD starts
			 if(s->streams[asf->stream_index]->codec->codec_id == AV_CODEC_ID_VC1)
            {
               /* Start Code Suffix - if Present */
               bdu[0] = avio_r8(pb);
               /* Start Code Prefix - if Present */
               bdu[1] = avio_r8(pb);
               bdu[2] = avio_r8(pb);
               bdu[3] = avio_r8(pb);

               /* If Frame Has Start Code */
               if((bdu[0] == 0x00)&&(bdu[1] == 0x00)&&(bdu[2] == 0x01))
               {
                  /* Frame Has Start Code. So no need to insert it again */
                  s->has_startcode = 1;
               }
...
#define AV_WL32(p, darg) do {                \
        ...
        if(offset == 4)
        {
          AV_WL32(asf_st->pkt.data + asf->packet_frag_offset,*(unsigned int *)bdu);
        }

		 ret = avio_read(pb, asf_st->pkt.data + asf->packet_frag_offset + offset,
                   asf->packet_frag_size - offset); 
