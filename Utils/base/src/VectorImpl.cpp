...
{
    if(mCount>0) printf( "[%p] subclasses of VectorImpl must call finish_vector() in their destructor. Leaking %d bytes.\n",
...
{
    if(mItemSize != rhs.mItemSize)
        printf("Vector<> have different types (this=%p, rhs=%p)\n", this, &rhs);
...
{
    if (index > size())
        return UTILS_BAD_INDEX;
    void* where = _grow(index, length);
    if (where) {
        _do_copy(where, array, length);
    }
    return where ? index : (ssize_t)UTILS_NO_MEMORY;
}
        ...
        while (i < count) {
            ...
            if (cmp(curr, item, state) > 0) {
                ...
                if (!temp) {
                    ...
                    array = editArrayImpl();
                    if (!array) return UTILS_NO_MEMORY;
                    temp = malloc(mItemSize);
                    if (!temp) return UTILS_NO_MEMORY;
...
{
    // binary search
    ssize_t err = UTILS_NAME_NOT_FOUND;
    ssize_t l = 0;
    ssize_t h = size()-1;
    ssize_t mid;
